...
class GrammarManager:
    ...
    def __init__(self, initial_grammar: Optional[str] = None):
        ...
        logger.info(f"Менеджер грамматики инициализирован с версией {self.current_version.version_id}")
    ...
    def parse(self, code: str) -> Tree:
        ...
        try:
            ...
            error_message = f"Синтаксическая ошибка: неожиданный токен '{e.token}' в позиции {e.pos_in_stream}"
            ...
            error_message = f"Синтаксическая ошибка: неожиданный символ '{e.char}' в позиции {e.pos_in_stream}"
            logger.error(error_message)
    ...
    def extend_grammar(self, new_rules: str, description: str, created_by: str = "manual") -> bool:
        ...
        update_matches = re.finditer(r'//\s*UPDATE_RULE:\s*(\w+)\s*\|=\s*(.+)$', new_rules, re.MULTILINE)
    ...
    def backup_all_versions(self, backup_dir: Union[str, Path]) -> bool:
        ...
        try:
            ...
            logger.info(f"Создана резервная копия {len(version_files)} версий грамматики в {backup_dir}")
            return True
